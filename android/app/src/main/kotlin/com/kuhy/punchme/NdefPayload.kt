...
/**
 * The MIME type identifying a punchme clock tag, for this flavor.
 *
 * Generated by Gradle from the same value the manifest placeholder uses, so
 * the intent filter and the record check cannot disagree. The sandbox flavor
 * carries its own type, which is what keeps a test tag from punching the real
 * timesheet.
 */
val PUNCH_MIME: String = BuildConfig.PUNCH_MIME
