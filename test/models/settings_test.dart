...
void main() {
  group('defaults', () {
    test('are 8h, Monday to Friday, no free days', () {
      ...
      expect(settings.workWifiSsids, isEmpty);
    });
  });
  ...
  group('copyWith', () {
    ...
    test('replaces the work Wi-Fi set', () {
      final changed = settings.copyWith(
        workWifiSsids: const <String>{'Office'},
      );
      expect(changed.workWifiSsids, const <String>{'Office'});
    });
    ...
    test('with no arguments keeps every field', () {
      ...
      expect(same.workWifiSsids, settings.workWifiSsids);
    });
  });
  ...
  group('json', () {
    test('round-trips a customised settings object', () {
      final original = const Settings().copyWith(
        ...
        workWifiSsids: const <String>{'Office', 'Annex'},
      );
      ...
      expect(restored.workWifiSsids, original.workWifiSsids);
    });
    ...
    test('writes sorted lists so the file is stable across writes', () {
      ...
      expect(json['workWifiSsids'], <String>['Annex', 'Office']);
    });
    ...
    test('falls back to defaults for missing fields', () {
      ...
      expect(restored.workWifiSsids, isEmpty);
    });
